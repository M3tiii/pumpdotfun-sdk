...
class PumpFunSDK {
    ...
    async loadTokenCreation(creator, mint, buyAmountSol, sellAmountSol, slippageBasisPoints = 500n, priorityFees, commitment = 'processed', finality = 'confirmed') {
        ...
        if (buyAmountSol > 0) {
            ...
            buyTx = await this.getBuyInstructions(creator.publicKey, mint.publicKey, globalAccount.feeRecipient, buyAmount, buyAmountWithSlippage);
        }
        let sellTx;
        if (sellAmountSol > 0) {
            const sellAmount = BigInt(Math.floor(Number(buyAmount) / (Number(buyAmountSol) / Number(sellAmountSol))));
            const sellAmountWithSlippage = (0, util_1.calculateWithSlippageSell)(sellAmountSol, slippageBasisPoints);
            sellTx = await this.getSellInstructions(creator.publicKey, mint.publicKey, globalAccount.feeRecipient, sellAmount, sellAmountWithSlippage);
        }
        ...
        const create = async (createTokenMetadata) => {
            ...
            let createResults = await (0, util_1.sendTx)(this.connection, newTx, creator.publicKey, [creator, mint], priorityFees, commitment, finality, creationBlockHash, true);
            return createResults;
        };
        const sell = async () => {
            let sellResults = await (0, util_1.sendTx)(this.connection, sellTx, creator.publicKey, [creator], priorityFees, commitment, finality, creationBlockHash, true);
            return sellResults;
        };
        return {
            create,
